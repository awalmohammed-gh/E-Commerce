...
import { loginAccount, registerAccount } from "../api/frontApis";
import toast from "react-hot-toast";

const AuthenticationForm = ({ onClose }) => {
  const { setIsLoggedIn, fetchUserData } = useECommerce();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const apiCalls =
        currentState === "login" ? loginAccount : registerAccount;
      ...
      >
        ...
        <div className="text-center pt-8 pb-4 px-6">
          <div className="w-16 h-16 bg-[#1E3A8A]/10 rounded-full flex items-center justify-center mx-auto mb-3">
            {currentState === "login" ? (
              <LogIn className="w-7 h-7 text-[#1E3A8A]" />
            ) : (
              <UserPlus className="w-7 h-7 text-[#1E3A8A]" />
            )}
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-6 pb-8">
          ...
          {currentState === "login" && (
            <div className="text-right">
              <button
                type="button"
                className="text-sm text-[#1E3A8A] hover:underline flex items-center justify-end gap-1"
              >
                Forgot Password? <ArrowRight size={12} />
              </button>
            </div>
          )}
          ...
          <button
            type="submit"
            className="bg-[#1E3A8A] text-white font-semibold py-2.5 rounded-xl hover:bg-[#2E4A9A] transition-all duration-200 mt-2 flex items-center justify-center gap-2"
          >
            {currentState === "login" ? (
              <>
                <LogIn size={18} />
